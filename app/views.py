...
def index():
	...
	if request.method == "POST":
		...
		imagename = image_upload.filename # upload image
		...
		image = image.resize((450,250)) #resize
		image_arr = np.array(image) # conver np array
		grey = cv2.cvtColor(image_arr,cv2.COLOR_BGR2GRAY) # convert to grey scale

	    #Cascade
		car_cascade = cv2.CascadeClassifier(car_cascade_src)  #train the model
		cars = car_cascade.detectMultiScale(grey, 1.1, 1) # apply the model

		bcnt = 0
		bus_cascade = cv2.CascadeClassifier(bus_cascade_src)
		bus = bus_cascade.detectMultiScale(grey, 1.1, 1)
		for (x,y,w,h) in bus:
			cv2.rectangle(image_arr,(x,y),(x+w,y+h),(0,255,0),2) # rectangle for buses
			bcnt += 1

		ccnt = 0
		if bcnt == 0:
			for (x,y,w,h) in cars:
				cv2.rectangle(image_arr,(x,y),(x+w,y+h),(255,0,0),2) # rectangle for cars
				ccnt += 1
